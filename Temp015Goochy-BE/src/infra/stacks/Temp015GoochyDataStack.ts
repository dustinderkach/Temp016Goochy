...
interface Temp015GoochyDataStackProps extends StackProps {
	allRegions: string[];
	envName: string;
}

export class Temp015GoochyDataStack extends Stack {
	...
	constructor(
		scope: Construct,
		id: string,
		props: Temp015GoochyDataStackProps
	) {
		super(scope, id, props);

		const primaryRegion = props.env?.region;
		const AppTableName = `${props.envName}-Temp015GoochyTable`;
	...
	private createAppPrimaryAndReplicaTables(
		...
		allRegions.forEach((region) => {
			...
			new CfnOutput(this, `${envName}-Temp015GoochyTableArn-${region}`, {
				value: arn,
