...
interface Temp015GoochyLambdaStackProps extends StackProps {
	tableArn: string;
	tableName: string;
	envName: string;
	bucketName: string;
...
export class Temp015GoochyLambdaStack extends Stack {
	public readonly temp015GoochyLambdaIntegration: LambdaIntegration;

	constructor(
		scope: Construct,
		id: string,
		props: Temp015GoochyLambdaStackProps
	) {
		...
		const temp015GoochyLambda = new NodejsFunction(
			this,
			`${props.envName}-Temp015GoochyLambda`,
			{
				runtime: Runtime.NODEJS_18_X,
				handler: "handlerTemp015Goochy",
				entry: join(
					__dirname,
					"..",
		...
		temp015GoochyLambda.addToRolePolicy(
			new PolicyStatement({
				effect: Effect.ALLOW,
		...
		this.temp015GoochyLambdaIntegration = new LambdaIntegration(
			temp015GoochyLambda
		);
