...
import { getTemp015Goochy as getTemp015Goochy } from "../../../src/services/temp015Goochy/GetTemp015Goochy";
...
describe("GetTemp015Goochy test suite", () => {
	const ddbClientMock = {
		send: jest.fn(),
	};
	...
	test("should return temp015Goochy if no queryStringParameters", async () => {
		ddbClientMock.send.mockResolvedValueOnce(someItems);
		const getResult = await getTemp015Goochy({} as any, ddbClientMock as any);
	...
	test("should return 400 if no id in queryStringParameters", async () => {
		const getResult = await getTemp015Goochy(
			{
	...
	test("should return 404 if no id in queryStringParameters", async () => {
		...
		const expectedResult = {
			statusCode: 404,
			body: JSON.stringify(`Temp015Goochy with id 123 not found!`),
		};
		expect(getResult).toEqual(expectedResult);
	});
