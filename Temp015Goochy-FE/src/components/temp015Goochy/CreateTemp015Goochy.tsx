...
export default function CreateTemp015Goochy({ dataService }: CreateTemp015GoochyProps) {
	...
	const handleSubmit = async (event: SyntheticEvent) => {
		...
		if (name && location) {
			const id = await dataService.createTemp015Goochy(
				name,
				location,
				photo
			);
			setActionResult(`Created temp015Goochy with id ${id}`);
			setName("");
			setLocation("");
		} else {
	...
	function renderForm() {
		...
		return (
			<form onSubmit={(e) => handleSubmit(e)}>
				...
				{renderPhoto()}
				<br />
				<input type="submit" value="Create temp015Goochy" />
			</form>
		);
	}
